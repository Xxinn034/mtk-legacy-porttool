import re
from .simg2img import is_sparse_image, simg2img
...
class portutils:
    ...
    def __ensure_raw_image(self, img_path):
        """
        检查镜像是否为 sparse image，如果是则自动转换为 raw 格式
        :param img_path: str, 镜像文件路径
        :return: str, 可能是原路径或转换后的 raw 镜像路径
        """
        if is_sparse_image(img_path):
            raw_path = img_path + ".raw"
            self.print_log(f"  检测到 Android sparse image，正在转换为 raw ext4 镜像...")
            self.print_log(f"  输入: {img_path}")
            self.print_log(f"  输出: {raw_path}")
            
            if simg2img(img_path, raw_path):
                self.print_log(f"  ✓ 转换成功: {raw_path}")
                return raw_path
            else:
                self.print_log(f"  ✗ 转换失败，请检查镜像文件是否完整")
                return img_path
        else:
            self.print_log(f"  镜像非 sparse 格式，直接处理")
            return img_path
